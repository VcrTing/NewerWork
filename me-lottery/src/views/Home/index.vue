...
function startLottery() {
  ...
  console.log('currentPrize =', currentPrize.value, currentPrize.value.isAll, 'notPersonList =', notPersonList.value, '')
...
async function stopLottery() {
  ...
  const lucyTargetAniTime = 1200 * 1
  const lucyTargetRotTime = 900 * 1

  console.log('stopLottery', lucyTargetAniTime)
...
<template>
  <div class="absolute z-10 flex flex-col items-center justify-center -translate-x-1/2 left-1/2">
    <h3
      ...
      :style="{ fontSize: `${textSize * 1.1}px`, color: textColor }"
    >
      {{ topTitle }}
    </h3>
  ...
  <div id="container" ref="containerRef" class="3dContainer">
    <!-- 选中菜单结构 start -->
    <div id="menu">
      <button v-if="currentStatus === 0 && tableData.length > 0" class="btn-enter" @click="enterLottery">
        {{ t('button.enterLottery') }}
      </button>

      <div v-if="currentStatus === 1" class="start">
        <button class="btn-enter" @click="startLottery">
      ...
      <button v-if="currentStatus === 2" class="btn-enter btn glass btn-lg" @click="stopLottery">
        {{ t('button.selectLucky') }}
      </button>

      <div v-if="currentStatus === 3" class="flex justify-center gap-6 enStop">
        <div class="start">
          <button class="btn-enter" @click="continueLottery">
        ...
        <div class="start">
          <button class="btn-enter btn-back" @dblclick="quitLottery">
