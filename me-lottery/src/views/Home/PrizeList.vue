...
<template>
  <div class="absolute left-0 o-prize-list-wrapper hide-scrollbar">
    <div class=" flex items-center">
      ...
      <div ref="prizeListContainerRef">
        <div v-if="temporaryPrize.isShow" class="o-prize-item-box w-72" :class="temporaryPrize.isShow ? 'current-prize' : ''">
          <div class="relative flex flex-row items-center justify-between w-full h-full shadow-xl card">
            <div
              v-if="temporaryPrize.isUsed"
              class="absolute z-50 w-full h-full bg-gray-800/70 item-mask rounded-xl"
            />
            <figure class="o-prize-img-wrap rounded-xl">
              <ImageSync v-if="temporaryPrize.picture.url" :img-item="temporaryPrize.picture" />
              <img v-else :src="defaultPrizeImage" alt="Prize" class="object-cover h-full rounded-xl o-prize-img ">
            </figure>
            <div class="items-center p-0 text-center card-body">
              <div class="tooltip tooltip-left" :data-tip="temporaryPrize.name">
                <h2 class="p-0 m-0 overflow-hidden w-28 o-prize-title whitespace-nowrap text-ellipsis">
                  {{
                    temporaryPrize.name }}
                </h2>
              </div>
        ...
        <transition name="prize-list" :appear="true">
          <div v-if="prizeShow && !isMobile && !temporaryPrize.isShow" class="flex items-center">
            <ul ref="prizeListRef" class="flex flex-col gap-1 p-2 rounded-xl bg-slate-500/50">
              ...
              >
              <div class="shadow-xl o-prize-item">
                  <div class="o-ps-fuii o-prize-item-bg">
                    <div class="o-prize-item-mask-inner card bg-base-100 o-prize-item-box">
                      <div>
                        ...
                        <img
                          ...
                          class="object-cover rounded-xl o-prize-img"
                        >
                    </div>
                  </div>
                  <div class="o-ps-fuii o-prize-item-mask card">

                  </div>
                  <div
                    v-if="item.isShow"
                    class="relative flex flex-row items-center justify-between o-prize-item-box card o-prize-item-card"
                  >
                    <div
                      v-if="item.isUsed"
                      class="absolute z-50 w-full h-full bg-gray-800/70 item-mask rounded-xl"
                    />
                    <div class="o-prize-img-wrap">
                      <div class="o-prize-img-inner rounded-xl">
                        <ImageSync v-if="item.picture.url" :img-item="item.picture" />
                        <img
                          v-else :src="defaultPrizeImage" alt="Prize"
                          class="object-cover rounded-xl o-prize-img"
                    ...
                    <div class="items-center p-0 text-center card-body">
                      <div class="tooltip tooltip-left" :data-tip="item.name">
                        <div
                          class="p-0 m-0 overflow-hidden text-center o-prize-title whitespace-nowrap text-ellipsis"
                        > {{ item.name }} </div>
                      </div>
                      <p class="absolute z-40 p-0 m-0 text-gray-300/80 mt-7 text-xs">
                        {{ item.isUsedCount }}/{{ item.count }}
                      </p>
                      <progress class="w-3/4 h-5 progress progress-primary" :value="item.isUsedCount"
                        :max="item.count" />
                      <!-- <p class="p-0 m-0">{{ item.isUsedCount }}/{{ item.count }}</p> -->
                    </div>
                  </div>
